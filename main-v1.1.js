document.addEventListener('DOMContentLoaded', () => {
  ...
  function buildDropdownItems(dropdown, cryptos, onSelect) {
    ...
    cryptos.forEach(coin => {
      ...
      networkDiv.style.display = 'inline-block';
      networkDiv.style.backgroundColor = networkColors[netKey] || '#444';
      networkDiv.textContent = network ? network.toUpperCase() : coin.symbol;
  ...
  function updateAmounts() {
    ...
    // Include to_currency in the call and lowercase them
  ...
  // Keep existing intervals or refresh logic as before
  // from previous code, if there's a 15-second interval, keep it:
  setInterval(() => {
    const fromAmount = parseFloat(fromAmountInput.value);
